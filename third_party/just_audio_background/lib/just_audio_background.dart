...
class SwitchAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
  AudioHandler _inner;
  ...
  SwitchAudioHandler(this._inner) {
    _subscribe();
  }

  set inner(AudioHandler value) {
    if (_inner != value) {
      _unsubscribe();
      _inner = value;
      _subscribe();
  ...
  void _subscribe() {
    _mediaItemSubscription = _inner.mediaItem.listen(mediaItem.add);
    _queueSubscription = _inner.queue.listen(queue.add);
    _playbackStateSubscription = _inner.playbackState.listen(playbackState.add);
    _customEventSubscription = _inner.customEvent.listen(customEvent.add);
  }
  ...
  @override
  Future<void> addQueueItem(MediaItem mediaItem) => _inner.addQueueItem(mediaItem);
  @override
  Future<void> addQueueItems(List<MediaItem> mediaItems) => _inner.addQueueItems(mediaItems);
  @override
  Future<void> insertQueueItem(int index, MediaItem mediaItem) => _inner.insertQueueItem(index, mediaItem);
  @override
  Future<void> updateQueue(List<MediaItem> queue) => _inner.updateQueue(queue);
  @override
  Future<void> updateMediaItem(MediaItem mediaItem) => _inner.updateMediaItem(mediaItem);
  @override
  Future<void> removeQueueItem(MediaItem mediaItem) => _inner.removeQueueItem(mediaItem);
  @override
  Future<void> removeQueueItemAt(int index) => _inner.removeQueueItemAt(index);
  @override
  Future<void> setRepeatMode(AudioServiceRepeatMode repeatMode) => _inner.setRepeatMode(repeatMode);
  @override
  Future<void> setShuffleMode(AudioServiceShuffleMode shuffleMode) => _inner.setShuffleMode(shuffleMode);
  @override
  ...
  @override
  Future<void> click([MediaButton button = MediaButton.media]) => _inner.click(button);
  @override
  Future<dynamic> customAction(String name, [Map<String, dynamic>? extras]) => _inner.customAction(name, extras);
  @override
  Future<void> onNotificationDeleted() => _inner.onNotificationDeleted();
}


const _toggleLikeAction = 'toggle_like';
const _toggleLikeRequestedEvent = 'toggle_like_requested';
const _skipNextRequestedEvent = 'skip_next_requested';
const _skipPreviousRequestedEvent = 'skip_previous_requested';
...
class _JustAudioBackgroundPlugin extends JustAudioPlatform {
  ...
  Future<AudioPlayerPlatform> init(InitRequest request) async {
    ...
    // For subsequent players, use the original platform
...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  bool get hasNextOverride => currentMediaItem?.extras?['hasNext'] == true;
  bool get hasPreviousOverride =>
      currentMediaItem?.extras?['hasPrevious'] == true;
  bool get showLikeControl =>
      currentMediaItem?.extras?['showLikeControl'] == true;
  ...
  Future<void> skipToNext() async {
    ...
    } else if (hasNextOverride) {
      customEvent.add(const {'event': _skipNextRequestedEvent});
    }
  }
  ...
  Future<void> skipToPrevious() async {
    ...
    } else if (hasPreviousOverride) {
      customEvent.add(const {'event': _skipPreviousRequestedEvent});
    }
  }
  ...
  Future<dynamic> customAction(String name, [Map<String, dynamic>? extras]) {
    if (name == _toggleLikeAction) {
      customEvent.add(const {'event': _toggleLikeRequestedEvent});
  ...
  void _broadcastState() {
    final canGoPrevious = hasPrevious || hasPreviousOverride;
    final canGoNext = hasNext || hasNextOverride;
    final controls = [
